...
use crate::{
    DEFAULT_CLIPBOARD_CAPACITY, DEFAULT_STT_MODEL, DEFAULT_HOTKEY, DEFAULT_HISTORY_HOTKEY,
    DEFAULT_SAMPLE_RATE, DEFAULT_VAD_SENSITIVITY, DEFAULT_VAD_TIMEOUT,
    MAX_CLIPBOARD_HISTORY, SUPPORTED_STT_MODELS, SUPPORTED_LANGUAGES
};
    ...
    pub fn validate(&self) -> crate::Result<()> {
        ...
        if !SUPPORTED_STT_MODELS.contains(&self.stt.model_size.as_str()) {
            return Err(crate::core::error::ConfigError::InvalidValue(
                format!("Unsupported STT model: {}", self.stt.model_size)
            ).into());
        }

        // Validate language if specified
        if !self.stt.language.is_empty() && !SUPPORTED_LANGUAGES.contains(&self.stt.language.as_str()) {
            return Err(crate::core::error::ConfigError::InvalidValue(
                format!("Unsupported language: {}", self.stt.language)
            ).into());
        }
        ...
        if self.clipboard.max_history > MAX_CLIPBOARD_HISTORY {
            return Err(crate::core::error::ConfigError::InvalidValue(
                format!("Clipboard history capacity too large: {}", self.clipboard.max_history)
            ).into());
        }
...
fn default_vad_sensitivity() -> f32 { DEFAULT_VAD_SENSITIVITY }
fn default_vad_timeout() -> u64 { DEFAULT_VAD_TIMEOUT }
fn default_noise_reduction() -> bool { true }
fn default_stt_backend() -> String { "local".to_string() }
fn default_stt_model() -> String { DEFAULT_STT_MODEL.to_string() }
fn default_enable_punctuation() -> bool { true }
fn default_enable_capitalization() -> bool { true }
fn default_clipboard_capacity() -> usize { DEFAULT_CLIPBOARD_CAPACITY }
fn default_auto_save() -> bool { true }
fn default_monitor_clipboard() -> bool { true }
fn default_max_item_size() -> usize { 10 * 1024 * 1024 } // 10MB
fn default_primary_hotkey() -> String { DEFAULT_HOTKEY.to_string() }
fn default_history_hotkey() -> String { DEFAULT_HISTORY_HOTKEY.to_string() }
fn default_quick_access_hotkeys() -> Vec<String> {
    vec!["Alt+1".to_string(), "Alt+2".to_string(), "Alt+3".to_string()]
}
fn default_enable_global_hotkeys() -> bool { true }
fn default_paste_mode() -> PasteMode { PasteMode::Both }
fn default_fallback_method() -> FallbackMethod { FallbackMethod::Clipboard }
fn default_restore_clipboard() -> bool { true }
fn default_paste_delay() -> u64 { 100 }
fn default_data_retention() -> String { "30d".to_string() }
fn default_auto_expiry() -> bool { true }
fn default_encrypt_storage() -> bool { true }
fn default_enable_analytics() -> bool { false }
fn default_show_tray() -> bool { true }
fn default_start_minimized() -> bool { false }
fn default_theme() -> String { "auto".to_string() }
fn default_enable_notifications() -> bool { true }
fn default_notification_duration() -> u64 { 5 }
...
impl HotkeyConfig {
    pub fn new() -> Self {
        Self {
            ...
            quick_access: vec!["Alt+1".to_string(), "Alt+2".to_string(), "Alt+3".to_string()],
            enable_global: true,
        }
    }
}
